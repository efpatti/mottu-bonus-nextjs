...
import { FaMotorcycle, FaInfoCircle, FaTrash, FaEdit } from "react-icons/fa";
...
const calculateBonus = (points: number) => {
 if (points < 5) return 0;
 return 40 + (points - 5) * 15;
};
...
const YourOwnCalculator = () => {
 ...
 const handleAddOrEditEntry = () => {
  ...
  if (!days || isNaN(total) || isNaN(paid) || paid > total) return;
 ...
 const handleCalculate = () => {
  const total = entries.reduce((acc, entry) => {
   const baseOption = daysOptions.find((o) => o.value === entry.days);
 ...
 const bonus = calculateBonus(totalPoints);
   ...
   <Card className="w-full max-w-md bg-white shadow-lg rounded-xl">
    <CardContent className="p-4 space-y-4">
     ...
     <div className="space-y-2">
      <div className="grid grid-cols-3 gap-2">
       ...
       <Button
        ...
        className="col-span-1 text-sm bg-green-600"
       >
     ...
     <div className="space-y-1 text-sm">
      {entries.map((entry) => {
       const label = daysOptions.find((d) => d.value === entry.days)?.label;
       return (
        <div
         key={entry.id}
        ...
        >
         <div>
          <strong>{label}</strong> — {entry.paidCount}/{entry.totalCount} pagas
         </div>
      ...
      <div className="flex items-start gap-2">
       ...
       <div className="text-xs">
        ...
        <ul className="list-disc pl-4 space-y-1 mt-1">
         <li>5 pontos = R$ 40,00</li>
         <li>Cada ponto adicional = +R$ 15,00</li>
         <li>Exemplo: 6 pontos = R$ 55,00 | 7 pontos = R$ 70,00</li>
        </ul>
       </div>
      </div>
     </div>

     <Button className="w-full bg-green-600" onClick={handleCalculate}>
      Calcular Bônus
     </Button>
      ...
      <div className="space-y-3">
       <div className="bg-green-50 p-3 rounded-lg border border-green-100 text-sm">
        <h3 className="font-semibold text-green-800 flex items-center gap-2 text-sm">
         <FaMotorcycle className="text-green-600" /> Resumo
        </h3>
        <p>
         Motos registradas: <strong>{totalMotos}</strong>
        </p>
        <p>
       ...
       {totalPoints < 5 && (
        <div className="bg-yellow-50 text-yellow-800 p-2 rounded-lg text-xs">
         Você precisa de {5 - totalPoints} ponto(s) adicional(is) para receber o
         bônus mínimo.
        </div>
       )}
      </div>
